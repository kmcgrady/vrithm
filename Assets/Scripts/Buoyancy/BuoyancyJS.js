	...
	var density : float = 300.0f;
	var slicesX : int = 3.0; // transformed according to dimension ratio with whole rigidbody bounds
	var slicesY : int = 3.0;
	var slicesZ : int = 3.0;
	...
	// Restore original rotation and position
		...
		var volume : float = GetComponent.<Rigidbody>().mass / density;

		voxels = WeldPoints(voxels, voxelsLimit);
	...
	function FixedUpdate()
	{
		...
		for (var point in voxels)
		{
			...
			if (wp.y - voxelHalfHeight < waterLevel)
			{
				...
				var velocity : Vector3 = transform.GetComponent.<Rigidbody>().GetPointVelocity(wp);
				var localDampingForce : Vector3 = -velocity * DAMPFER * GetComponent.<Rigidbody>().mass;
	...
	{
		var gizmoSize : float = 0.0005f * GetComponent.<Rigidbody>().mass;
